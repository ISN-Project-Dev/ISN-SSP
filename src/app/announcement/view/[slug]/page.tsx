    ...
    <>
      <div className="relative w-full">
        <Image
          ...
          height={200}
        />
        <h2 className="absolute inset-0 flex items-center justify-center text-[#192f59] text-3xl font-bold bg-blue-50/30">Announcement Details</h2>
      </div>
        ...
        <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300">
          <CardHeader className="bg-[#46587a] rounded-t-xl p-6">
            ...
            {isEventRelated && announcement.event && (
              <p className="mt-2 text-gray-200 text-sm opacity-80">Related to event: <strong>{announcement.event.title}</strong></p>
            )}
          </CardHeader>
          <CardContent className="p-6 bg-white">
            <p className="prose prose-lg text-gray-700">{announcement.description}</p>
          </CardContent>
          <CardFooter className="flex justify-between items-center bg-gray-100 rounded-b-xl px-6 py-4">
            <Link href="/announcement">
              <Button variant="ghost" size="sm">← Back to All</Button>
            </Link>
