      ...
      <div className="flex max-w-[1440px] mx-auto items-center justify-between px-5 py-3 text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-5">
            ...
            />
            <h1 className="text-base font-semibold hidden sm:block">SKS Student Portal</h1>
          </div>
        </div>
        <div className="hidden md:flex gap-6 text-sm font-semibold">
          {navLinks.map(({ title, url }, i) => (
            <Link
              key={i}
              href={url}
              className="px-3 hover:text-gray-300 transition"
            >
              {title}
            </Link>
          ))}
        ...
        <div className="relative">
          {currentUser ? (
            ...
            <Link href="/auth/login">
              <button className="items-center text-sm px-5 py-2 rounded-lg bg-white text-[#192f59] hover:bg-gray-300 focus:ring-1 focus:ring-[#2f4369] focus:ring-offset-1">Login</button>
            </Link>
          )}
        </div>
      </div>
